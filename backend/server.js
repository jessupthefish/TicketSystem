...
const pool = new Pool({
    ...
    ssl: process.env.NODE_ENV === "production" ? { rejectUnauthorized: false } : false // Enable SSL for cloud-hosted databases
});
...
app.get('/api/tickets', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM tickets');  // Assuming a 'tickets' table in your DB
...
app.post('/api/tickets', async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO tickets (title, description) VALUES ($1, $2) RETURNING *', [title, description]);
...
app.put('/api/tickets/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query('UPDATE tickets SET title = $1, description = $2 WHERE id = $3 RETURNING *', [title, description, id]);
